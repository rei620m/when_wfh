...
future_data['weekday_sin'] = np.sin(2 * np.pi * future_data['weekday'] / 7)
future_data['weekday_cos'] = np.cos(2 * np.pi * future_data['weekday'] / 7)
...
future_predictions = rf_model.predict(future_data.drop(columns=['date', 'wfh']))
future_data['wfh_prediction'] = future_predictions.astype(int)

print(future_data[['date', 'wfh_prediction']])
